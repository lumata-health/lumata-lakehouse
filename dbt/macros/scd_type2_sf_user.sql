{% doc scd_type2_sf_user %}
This macro generates Type 2 Slowly Changing Dimension (SCD) records for the `sf_user` table. It is designed to be called by the `scd_merge_sf_user` macro.

**Arguments:**
- None

**Behavior:**
1. **Reads Staging Data:** It reads the data from the `stg_sf_user` staging model.
2. **Detects Changes:** It detects changes in the `division` and `audit_phase__c` fields.
3. **Generates SCD Records:** It generates new SCD records for each change, with appropriate `is_current` and `is_deleted` flags.
4. **Assigns Surrogate Keys:** It assigns a surrogate key (`user_key`) to each SCD record.

**Returns:**
A CTE that contains the generated SCD records.
{% enddoc %}
